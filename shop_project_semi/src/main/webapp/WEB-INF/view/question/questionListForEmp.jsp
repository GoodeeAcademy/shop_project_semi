	...
	<div class="col-sm-10 col-md-8 col-lg-6 m-lr-auto" style="margin-top: 5%; margin-bottom: 5%;">
		...
		<div class="flex-c-m flex-w w-full p-t-38">
			<c:if test="${currentPage > 10}">
				<a href="${pageContext.request.contextPath}/QuestionListForEmpController?currentPage=${startPage-10}" class="flex-c-m how-pagination1 trans-04 m-all-7">이전</a>
			</c:if>
			<c:forEach var="i" begin="${startPage}" end="${endRow}">
				<c:if test="${i == currentPage}">
						<a href="${pageContext.request.contextPath}/QuestionListForEmpController?currentPage=${i}" class="flex-c-m how-pagination1 trans-04 m-all-7 active-pagination1">${i}</a>
				</c:if>
				<c:if test="${i != currentPage}">
						<a href="${pageContext.request.contextPath}/QuestionListForEmpController?currentPage=${i}" class="flex-c-m how-pagination1 trans-04 m-all-7">${i}</a>
				</c:if>
			</c:forEach>
			<c:if test="${beginRow+10 < lastPage}">
				<a href="${pageContext.request.contextPath}/QuestionListForEmpController?currentPage=${startPage+10}" class="flex-c-m how-pagination1 trans-04 m-all-7">다음</a>
			</c:if>
		</div>
	</div>
